            ...
            >
              ...
              <template v-else>
                <h6 class="m-0 font-weight-bold text-">
                  Data {{ paket_text.nama_paket }}
                </h6>
              </template>
              <Link
                :href="'/member/paket_aktif/' + id_paket + '/text'"
                class="btn btn-light"
              >
                ...
                <div class="col-sm-3" v-for="paket in paket_text">
                  <div class="card p-0 overflow-hidden border-0 shadow-sm">
                    <div class="card-body p-0">
                      ...
                      <div
                        class="p-3 d-flex justify-content-between align-items-center flex-row"
                      >
                        <small>{{ paket.text.judul_materi }}</small>
                        <a
                          :href="paket.text.link"
                          target="_blank"
                          class="btn main-radius btn-outline-primary"
                          >buka</a
                        >
                      </div>
                    </div>
...
export default {
  ...
  methods: {
    fetchDataPaket() {
      this.loading = true;
      axios
        .get(`/api/get-text-paket/${this.id_paket_text}`)
        .then((response) => {
