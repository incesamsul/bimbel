...
<template>
  ...
  <AuthenticatedLayout :user="user">
    <div class="container-fluid">
      ...
      <p class="mb-4">
        Lakukan penambahan/pengeditan paket dengan mengisi form dibawah ini.
      </p>
      ...
      <div class="card border-0 mb-4">
        ...
        >
          ...
          <Link href="/paket" class="btn btn-light">
        ...
        <div class="card-body">
          ...
          <div class="form-group" v-if="inputJenisPaket == 'bimbel'">
            <label for="link_telegram">link Zoom</label>
            <input
              v-model="inputLinkZoom"
              type="text"
              class="form-control"
              name="link_zoom"
              id="link_zoom"
                    ...
                    <div class="card-body p-5">
                      <div class="row">
                        <div
                          ...
                          class="col-6 mr-3 col-sm-4 col-md-2 folder-col"
                          v-for="item in paket_menu"
                          :key="item.id"
                        >
                          <i class="fas fa-folder text-info"></i>
                          <p>{{ item.name }}</p>
                          <!-- Assuming there's a name property in your items -->
                        </div>
                      </div>
                    </div>
                  </div>
...
export default {
  data() {
    return {
      ...
      selectedMenu: [],
      paket_menu: [
        { id: 1, name: "materi_text", selected: false },
        { id: 2, name: "materi_video", selected: false },
        { id: 3, name: "tryout", selected: false },
        { id: 4, name: "latihan", selected: false },
        { id: 5, name: "group_telegram", selected: false },
        { id: 5, name: "link_zoom", selected: false },
      ],
    };
  },
  props: {
  ...
  methods: {
    toggleMenu(item) {
      ...
      if (item.selected) {
        this.selectedMenu.push(item); // Add the selected item to the array
      } else {
        const index = this.selectedMenu.findIndex((el) => el.id === item.id); // Find the index using unique identifier
        if (index !== -1) {
          this.selectedMenu.splice(index, 1); // Remove the unselected item from the array
        }
      }
    },
    toggleVideo(item) {
    ...
    submitpaket() {
      const formData = {
        ...
        isi_paket: this.selectedMenu.map((obj) => obj.name).join(", "),
  ...
  mounted() {
    ...
    if (this.edit) {
      ...
      this.inputLinkZoom = this.edit.link_zoom;
      ...
      this.paket_menu.forEach((item) => {
        let isiPaketArr = this.edit.isi_paket
          ? this.edit.isi_paket.split(", ")
          : [];
        if (isiPaketArr.includes(item.name)) {
          item.selected = true;
